...
- ==题解：==
  ...
  3. ==当最大和最小牌<5时返回true==
    - ==既没有牌相等，同时也有joker替换==
    - 此时nums[4]即为最大数，nums[joker]即为最小数
...
- 三刷理解：
  1. 初始化dp边界，即dp[6]，fill为1/6.0
  2. 第一层循环：骰子数量从2开始
  3. 第二层循环：[0,dp.length]，此层可看作是不同滑动窗口的起点
  4. 第三层循环：[0,6]，此层可看作是滑动窗口固定大小
    - 比如骰子为2时，滑动窗口从第0个开始向右，每次将窗口内的所有元素加上窗口起点即dp[j]/6.0的概率。此时第0个只累加执行了一次，而中间的6和7则都执行了五次。
...
- ==dp含义：第i个丑数为dp[i]==
- ==dp边界：最小的丑数为1，也是dp的丑数计算的边界即dp[0]=1==
- ==题解：==
  1. 初始化p1,p2,p3=0，分别对应质因子为2,3,5的丑数计数器
  2. 遍历第二到第n个丑数进行计算
    1. 计算2,3,5对应的丑数
      - 计算方式：丑数各自指针乘以质因数如2*dp[i2]
    2. 状态转移公式：
      - 由于求的是第n位丑数，所以取min值才是连续位置的丑数
    3. 更新2,3,5对应丑数计数器，更新条件是等于第i个丑数
  3. 返回第n个丑数即dp[n-1]

#### 数值的整数次方 剑16
#### 方法一：快速幂
- ==重点：快速幂，就是将【次方】以二进制的形式看待，当次方对应的位为1时保存乘积x，即res*=x，否则不保存，其中x不断相乘即x*=x==
  - 如：如n=18（10010），x*=x，n>>=1，当为n%2=1时x保存进res
- 方案：
  1. ==初始化res=1，使用long b替代n避免负数(负数比整数多1)转整数时越界==
  2. ==当次方为负数时需要做转换==
    1. x=1/x
    2. n=-n
  3. ==当次方n不为0时==
...
#### 方法一：归并排序
- ==在合并的过程中，当左元素大于右元素时==
  - ==count+=mid-lIndex+1，因为(lIndex,mid]的元素大于lIndex位的元素，即左边元素都满足逆序==
...
- 如何找到BST的root节点呢？以及左右子树各自的元素范围呢？
  - 最后一个元素nums[right]即为root节点
  - 左右范围确定，初始化p=left：
    1. 通过从nums[p]<nums[right](根节点)，p++，直到遇到nums[p]>=root节点为止，保存mid=p，此时左子树范围[left,mid-1]
    2. 再通过nums[p]>nums[right](根节点)，p++，直到不满足条件，右子树范围[mid,right-1]
      - 因为right是根节点，所以需要-1
    3. 如整个过程中，左子树范围中穿插着某个元素nums[p]>=nums[right]或者右子树范围穿插着某个元素nums[p]<=nums[right]，那么就不符合BST左小右大的特性，就不是BST。
    4. 当最后p=right时，说明遍历了所有元素都符合条件，那么就是BST。递归左右子树继续判断奇[left,mid-1]和[mid,right-1]
- 方案：不断找到左右子树
  1. 传入参数(nums,0,len-1)
  2. 当left>=right，直接返回true
    - 不是=是因为存在right-1后使得left>right的情况，此时同样返回true
  3. 设定p=left
  4. ==遍历nums[p]<nums[right]，满足则p++，找到当前子树root节点==
  5. ==遍历nums[p]>nums[right]，满足则p++，判断是否为BST，如p=right说明符合BST性质，否则说明存在节点不满足BST性质==
    1. 当p=right时，递归左右区间
    2. 当p!=right直接返回false
